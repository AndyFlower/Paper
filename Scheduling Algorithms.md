...
**permutation flflow shop**：置换流程车间
...
机器环境的特点是由两个参数组成的字符串α=α1α2 ，α的可能值是o,P, Q, R, PMPM, QMPM, G, X,O, J, F.如果α1∈{◦, P, Q, R, PMPM, QMPM}，其中o代表一个空符号（如果α1=o则表示α=α2），则表示工作J<sub>i</sub>由一个单一的操作组成。

如果α1=◦，则必须在指定的专用机器上处理每个作业。 如果α1∈{p，q，r}，那么我们有并行机器，即。 每个作业都可以在M<sub>1</sub>，...，M<sub>m</sub>每台机器上处理。 

如果α1=p，那么就有相同的并行机器。

因此，对于M<sub>J</sub>上作业ji的处理时间P<sub>IJ</sub>，我们对所有机器M<sub>J</sub>都有P<sub>IJ</sub>=P<sub>I</sub>。

如果α1=Q，那么就有统一的并行机器，即。 PIJ=PI/SJ，其中SJ是机器MJ的速度。 

最后，如果α1=r，那么就有不相关的并行机器，即。 PIJ=PI/SiJ对MJ的工作依赖速度Sij。如果α1=pmpm和α1=qmpm，那么我们分别有相同和均匀速度的多用途机器

如果α1∈{g，x，o，j，f}，我们有一个多操作模型，即。 与每个作业ji相关联的是一组操作oi1，...，oi，ni。 这些机器是专用的，即。 所有μij都是一个元素集。 此外，任意操作之间存在优先关系。 这种通用模型被称为普通商店。 我们通过设置α1来指示一般商店=例如工作商店、流动商店、开放商店和混合商店是一般商店的特例。在由α1=j表示的工作商店中，我们有特殊的形式优先关系

![image-20200922104742478](images\image-20200922104742478.png)

此外，我们通常假设μ<sub>ij</sub>!=μ<sub>i，j+1</sub>其中j从1到n<sub>i</sub>-1。我们称一个u<sub>ij</sub>=u<sub>i,j+1</sub>的为有机器重复的工作车间。 

流动车间，由α1=f表示，是作业车间的一个特例，其中i=1....n的时候n<sub>i</sub>=m ;i=1...n j=1...m的时候u<sub>ij</sub>={M<sub>j</sub>}

α=O的时候定义的开放商店是流动商店的一个特例，这个特例是机器之间没有优先关系的。

α1=X表示的是混合商店，它是一个工作商店和开放商店的结合。

置换流程车间是一个流程车间，在每个机器上以相同的顺序处理作业。 

![image-20200922110200124](images\image-20200922110200124.png)

如果我们有一个工作商店的问题，我们可以设置β4等于n<sub>i</sub>≤2.在这种情况下，所有的工作最多有两个操作。

如果α2等于正整数1，2，...，那么α2表示机器的数量。 如果α2=k，那么k是一个任意的，但固定数量的机器。 如果机器的数量是任意的，我们设置α2=◦。

### 1.5  **Optimality Criteria** 最优性标准

我们使用C<sub>i</sub>来表示工作J<sub>i</sub>的完成时间，用f<sub>i</sub>（C<sub>i</sub>）来表示相关的成本。基本上有两种类型的总成本函数 

![image-20200922110911841](images\image-20200922110911841.png)

分别叫做瓶颈目标和总目标

调度问题是找到一个可行的调度，使总成本函数最小化。 

如果没有指定函数f<sub>i</sub>，我们设置γ=f<sub>max</sub>或者γ= ∑f<sub>i</sub>

然而，在大多数情况下，我们考虑特殊函数f<sub>i</sub>。

最常见的目标函数是makepan max{C<sub>i</sub>|i=1，...，n}，总流动时间![image-20200922111619778](images\image-20200922111619778.png)加权（总）流动时间 ![image-20200922111654474](images\image-20200922111654474.png)这种情况下我们分别设置![image-20200922111801265](images\image-20200922111801265.png)

其他目标函数取决于与作业j<sub>i</sub>相关的到期日d<sub>i</sub>。 

我们分别定义每一个工作J<sub>i</sub>为：

![image-20200922112522998](images\image-20200922112522998.png)

absolute deviation 绝对偏差

squared deviation 平方偏差

unit penalty 单位惩罚

利用这些函数G<sub>i</sub>，我们得到了四个可能的目标γ=maxG<sub>i</sub>，maxwiG<sub>i</sub>，∑G<sub>i</sub>，∑w<sub>i</sub>g<sub>i</sub>。 

最重要 瓶颈目标除了C<sub>max</sub>就是最近最大![image-20200922113158147](images\image-20200922113158147.png)其他最长使用的目标函数是![image-20200922113248559](images\image-20200922113248559.png)![image-20200922113315914](images\image-20200922113315914.png)

这些目标函数的线性组合也是需要考虑的。

一个相对于所有变量C<sub>i</sub>不递减的目标函数称为正则函数。 与E<sub>i</sub> D<sub>i</sub> S<sub>i</sub>相关的不是正则函数。目前为止定义的其他函数是正则的。

如果不可能在不违反某些约束的情况下提前调度作业(操作)，则调度称为活动调度。

如果没有作业（操作）可以在不改变处理顺序或违反约束的情况下更早地处理，则调度称为半主动。

### 1.6 Examples

为了说明α|β|γ我们给出了一些例子。在每个例子中我们会描述问题。此外，我们将指定一个实例，并以甘特图的形式为实例提供一个可行的时间表。 

例子1.1 P|prec；p<sub>i</sub>=1|C<sub>max</sub>是在m个相同的机器上调度具有单元处理时间和任意优先约束的作业的问题，使得最长完工时间最小化。 

实例由具有n个顶点和机器数的有向图给出。

![image-20200922163539004](images\image-20200922163539004.png)

示例1.2  1|s-batch|∑w<sub>i</sub>C<sub>i</sub>是将一组作业分解成批处理，并在一台机器上调度这些批处理，使加权流时间最小化。批处理时间是批处理中作业的处理时间之和。

![image-20200922164242031](images\image-20200922164242031.png)

![image-20200922164942665](images\image-20200922164942665.png)

![image-20200922165234598](images\image-20200922165234598.png)